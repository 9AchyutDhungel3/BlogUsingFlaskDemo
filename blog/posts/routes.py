...
def update_post(post_id):
    post = Post.query.get_or_404(post_id) # see if the post with that particular id exists in the database.
    ...
    return render_template('create_post.html', title=post.title, form=form)  # Here we pass the form (which we have configured to be prefilled with previous data from db..)
